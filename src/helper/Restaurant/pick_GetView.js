...
const pick_GetView = async (rest_id, user_lat, user_long,user_id) => {
  try {
    let restaurant, populer_menu;

    restaurant = await RestaurentModel.aggregate([
      { $match: { _id: rest_id ,is_active:true,is_pickup:true} },
      {
        $lookup: {
          ...
          pipeline: [
            { $match: { customer_id: user_id ,is_pickup:true} },
          ],
          as: "is_favourite",
